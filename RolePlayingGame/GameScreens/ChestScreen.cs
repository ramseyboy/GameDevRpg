...
internal class ChestScreen : InventoryScreen
{
    ...
    /// </summary>
    private readonly MapEntry<Chest> chestEntry;
    ...
    public bool IsGoldSelected =>
        chestEntry != null && chestEntry.Content != null &&
    ...
    protected override void MoveCursorRight()
    {
        ...
        selectedQuantity = selectedQuantity < maximumQuantity ? selectedQuantity + 1 : 0;
    }
        ...
        : base(true)
    {
        ...
        this.chestEntry.Content.Entries.RemoveAll(
            delegate(ContentEntry<Gear> contentEntry) { return contentEntry.Count <= 0; });
    ...
    public override void Draw(GameTime gameTime)
    {
        ...
        var maximumCount = chestEntry.Content.Gold > 0 ? dataList.Count + 1 : dataList.Count;
        ...
        for (var index = StartIndex; index < EndIndex; index++)
        {
            ...
            {
                var currentIndex = chestEntry.Content.Gold > 0 ? index - 1 : index;
                var entry = dataList[currentIndex];
                if (index == SelectedIndex)
                {
                    DrawSelection(position);
    ...
    protected virtual void DrawGoldEntry(Vector2 position, bool isSelected)
    {
        ...
        spriteBatch.Draw(goldTexture,
            drawPosition + iconOffset + new Vector2(0f, 7f),
            Color.White);
        ...
        if (isSelected)
        {
            ...
            spriteBatch.Draw(leftQuantityArrow,
                new Vector2(drawPosition.X, drawPosition.Y - 4),
                Color.White);
            drawPosition.X += leftQuantityArrow.Width;
            // draw the selected quantity ratio
            var quantityText = selectedQuantity + "/" +
                               chestEntry.Content.Gold;
