...
public class Party
{
    ...
    public void JoinParty(Player player)
    {
        ...
        foreach (var contentEntry in player.Inventory)
        {
    ...
    public void GiveExperience(int experience)
    {
        ...
        {
            var oldLevel = player.CharacterLevel;
    ...
    /// </summary>
    [ContentSerializerIgnore]
    public ReadOnlyCollection<ContentEntry<Gear>> Inventory => inventory.AsReadOnly();
    ...
    public void AddToInventory(Gear gear, int count)
    {
        ...
        var existingEntry = inventory.Find(
            delegate(ContentEntry<Gear> entry) { return entry.Content == gear; });
    ...
    public bool RemoveFromInventory(Gear gear, int count)
    {
        ...
        existingEntry.Count -= count;
        var fullRemoval = existingEntry.Count >= 0;
    ...
    /// </remarks>
    private readonly Dictionary<string, int> monsterKills = new();
    ...
    ///     Used to determine if the requirements for the active quest have been met.
    /// </remarks>
    public Dictionary<string, int> MonsterKills => monsterKills;
        ...
        ContentManager contentManager)
    {
        ...
        // load the players
        foreach (var contentName in gameStartDescription.PlayerContentNames)
        {
    ...
    public Party(PartySaveData partyData, ContentManager contentManager)
    {
        ...
        // load the players
        foreach (var playerData in partyData.players)
        {
            var player =
            ...
            foreach (var equipmentAssetName in playerData.equipmentAssetNames)
            {
        ...
        for (var i = 0; i < partyData.monsterKillNames.Count; i++)
        {
